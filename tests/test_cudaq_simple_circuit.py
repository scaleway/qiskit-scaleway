...
def test_cudaq_simple_circuit():
    ...
    backend = provider.get_backend(
        os.getenv("CUDAQ_SCALEWAY_BACKEND_NAME", "EMU-CUDAQ-64C-512M")
    )

    assert backend is not None

    session_id = backend.start_session(
        name="my-cudaq-session-autotest",
        deduplication_id=f"my-cudaq-session-autotest-{random.randint(1, 1000)}",
        max_duration="15m",
    )
    ...
    try:
        ...
        assert result.results[0].shots == shots_count
